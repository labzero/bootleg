...
task :local_compile do
  ...
  File.cd!(source_path, fn ->
    System.cmd "mix", ["local.rebar", "--force"], env: [{"MIX_ENV", mix_env}], into: IO.stream(:stdio, :line)
    System.cmd "mix", ["local.hex", "--force"], env: [{"MIX_ENV", mix_env}], into: IO.stream(:stdio, :line)
    System.cmd "mix", ["deps.get", "--only=#{mix_env}"], env: [{"MIX_ENV", mix_env}], into: IO.stream(:stdio, :line)
    System.cmd "mix", ["deps.compile"], env: [{"MIX_ENV", mix_env}], into: IO.stream(:stdio, :line)
...
task :local_copy_release do
  ...
  archive_path = Path.join(source_path, "_build/#{mix_env}/rel/#{app_name}/releases/#{app_version}/#{app_name}.tar.gz")
