defmodule BootlegTest do
  ...
  test "check_config" do
    assert Bootleg.check_config(%Bootleg.Config.BuildConfig{}, ~w()) == :ok

    # strategies can require keys to be set (non-nil)
    config = %Bootleg.Config.BuildConfig{host: nil}
    assert {:error, _} = Bootleg.check_config(config, ~w(host))

    config = %Bootleg.Config.BuildConfig{host: "acme.local"}
