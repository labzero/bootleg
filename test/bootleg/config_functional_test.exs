defmodule Bootleg.ConfigFunctionalTest do
  ...
  test "remote/2 multiple roles" do
    ...
    task :remote_multiple_roles_multiple_commands do
      ...
      assert [
               [{:ok, [stdout: hostname_1], 0, _}, {:ok, [stdout: hostname_2], 0, _}],
               [{:ok, [stdout: foo_1], 0, _}, {:ok, [stdout: foo_2], 0, _}]
             ] = out
  ...
  test "remote/2 multiple roles/hosts" do
    capture_io(fn ->
      ...
      assert [
               [{:ok, [stdout: host_1], 0, _}, {:ok, [stdout: host_2], 0, _}],
               [{:ok, [stdout: host_3], 0, _}]
  ...
  test "download/3" do
    capture_io(fn ->
      ...
      assert {:error, :enotdir} =
               File.read(Path.join(to_download_path, "download_force_current_dir"))
